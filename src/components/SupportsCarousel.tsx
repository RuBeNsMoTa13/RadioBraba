...
>(
  (
    ...
    const [carouselRef, api] = useEmblaCarousel(
      ...
      [Autoplay({ delay: 3000, stopOnInteraction: false }) as EmblaPluginType]
    );
    ...
    // Padronização dos caminhos de imagens começando com "/"
    ...
    return (
      ...
      >
        <div
          ...
          {...props}
        >
        <h1 className="text-center text-primary font-bold text-2xl pb-8 mt-2">Apoiadores</h1>
...
>(({ className, ...props }, ref) => { // Mantendo className pois é usado no cn()
  ...
  return (
    <div ref={carouselRef} className="overflow-hidden">
      <div
        ...
        className={cn(
          "flex",
          orientation === "horizontal" ? "md:gap-x-2" : "gap-y-4 flex-col",
          className
        )}
        {...props}
      />
...
>(({ variant = "outline", size = "icon", ...props }, ref) => { // Removendo className não utilizada
  const { scrollPrev, canScrollPrev } = useSupportsCarousel(); // Removendo orientation não utilizada
...
const SupportsCarouselNext = React.forwardRef<
  ...
  React.ComponentProps<typeof Button>
>(({ variant = "outline", size = "icon", ...props }, ref) => { // Removendo className não utilizada
  const { scrollNext, canScrollNext } = useSupportsCarousel(); // Removendo orientation não utilizada
