...
const Announcer: React.FC = () => {
    ...
    const openModal = (locutor: Locutor) => setSelectedLocutor(locutor);
    const closeModal = () => setSelectedLocutor(null);
                ...
                <div className="overflow-hidden" ref={emblaRef}>
                    <div className="flex">
                        {locutoresData.map((Locutor) => (
                            ...
                            >
                                <div
                                    className="relative bg-gray-800 rounded-lg overflow-hidden h-96 group-hover:transform group-hover:scale-105 transition-transform duration-300 cursor-pointer"
            ...
            {selectedLocutor && (
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
                    <div className="bg-card rounded-lg max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col md:flex-row">
                        ...
                        <button
                            onClick={closeModal}
                            className="absolute top-4 right-4 z-[70] bg-black/50 hover:bg-black/70 rounded-full p-2 transition-colors"
                        >
                            <X className="w-6 h-6" />
                        </button>
                            ...
                            <div className="absolute inset-0 opacity-20">
                                <div className="flex flex-wrap gap-2 p-2">
                                    {[...Array(20)].map((_, i) => (
                                        <div
                                            key={i}
                                            className="w-12 h-12 rounded-full border border-pink-500 flex items-center justify-center"
                                        >
                                        </div>
                                    ))}
                                </div>
                        ...
                        <div className="md:w-1/2 p-8 overflow-y-auto">
                            ...
                            <h2 className="text-3xl font-bold mb-2 text-primary">{selectedLocutor.nome}</h2>
